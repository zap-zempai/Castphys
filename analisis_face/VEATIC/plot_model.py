...
def comput_va(path_model, frames, N=300):
    ...
    with torch.no_grad():
        for i in tqdm(range(0,len(frames),N)):
            ...
            model_output = model(frame).detach().cpu().numpy()
            val_output.extend(model_output[:,-2:]) # valece & arousal
            emotion_output.extend([idx_to_class[i] for i in np.argmax(model_output[:,:num_classes], axis=1)]) # emotion class
...
if __name__ == "__main__":
    ...
    id =45
    v_name = 'Q7_2'
    patient_folder = f"D:/castphys_60/Data/Patient_{id}/{v_name}/"
    ...
    valence, arousal, emotion = comput_va(path_model, frames, N=300)
    valence = valence*2
    arousal = arousal*2
