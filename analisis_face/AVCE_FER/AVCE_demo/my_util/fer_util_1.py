...
alexnet   = pretrainedmodels.__dict__['alexnet'](num_classes=1000, pretrained=None).to(device)
resnet    = pretrainedmodels.__dict__['resnet18'](num_classes=1000, pretrained=None).to(device)


def nn_output():
    encoder2 = Encoder_R18().to(device)
    regressor= Regressor_R18().to(device)
...
class Regressor_R18(nn.Module):
    ...
    def __init__(self):
        ...
        self.avgpool = resnet.avgpool.to(device)
        self.last_linear = resnet.last_linear.to(device)
        self.lin0 = nn.Linear(1000, 32).to(device)
        self.lin1 = nn.Linear(32, 256).to(device)
        self.va_regressor = nn.Linear(256, 2).to(device)
...
class Regressor_R18_Category(nn.Module):
    ...
    def __init__(self):
        ...
        self.lin0 = nn.Linear(1000, 32).to(device)
        self.lin1 = nn.Linear(32, 256).to(device)
        self.va_regressor = nn.Linear(256, 7).to(device)
        self.sigmoid = nn.Sigmoid()
...
class Regressor_MMx(nn.Module):
    ...
    def __init__(self):
        ...
        self.avgpool = resnet.avgpool.to(device)
        self.last_linear = resnet.last_linear.to(device)
        self.lin0 = nn.Linear(64, 32).to(device)
        self.lin1 = nn.Linear(32, 256).to(device)
        self.va_regressor = nn.Linear(256, 2).to(device)
...
if __name__ == "__main__":
    ...
    #print(fg256("yellow", summary(Encoder_Alex(), torch.ones_like(torch.empty(1, 3, 255, 255)).to(device), show_input=True)))
    #print(fg256("cyan", summary(Encoder_R18(), torch.ones_like(torch.empty(1, 3, 255, 255)).to(device), show_input=True)))
    #print(fg256("green", summary(Regressor_Alex(), torch.ones_like(torch.empty(1, 256, 6, 6)), show_input=True)))
    #print(fg256("orange", summary(Regressor_R18(), torch.ones_like(torch.empty(1, 512, 8, 8)).to(device), show_input=True)))
